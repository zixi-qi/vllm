...
class _ShardDescLayout:
    ...
    @property
    def num_fa_descs(self) -> int:
        return self.num_regions * self.num_blocks
...
def _build_layer_name_to_kv_group_index(
    ...
    for group_idx, group in enumerate(kv_cache_config.kv_cache_groups):
        for layer_name in group.layer_names:
            if layer_name in result:
                raise ValueError(
                    f"KV cache layer {layer_name!r} appears in multiple "
                    "kv_cache_groups."
                )
            result[layer_name] = group_idx
    return result
...
class NixlConnectorWorker:
    ...
    def __init__(
        ...
        self._layer_name_to_kv_group_index = _build_layer_name_to_kv_group_index(
            kv_cache_config
        )
        ...
        self.local_registered_layer_indices: list[int] = []
        self.local_registered_layer_names: list[str] = []
        ...
        self.dst_xfer_side_handles = defaultdict[
            EngineId, dict[tuple[int, int], int]
        ](dict)
        self._xfer_desc_layouts: dict[
            tuple[EngineId, int, str], _ShardDescLayout
        ] = {}
    ...
    def _ensure_local_registered_layer_names(self) -> list[str]:
        local_names = getattr(self, "local_registered_layer_names", None)
        if local_names:
            return local_names

        local_layers = getattr(self, "local_registered_layer_indices", None)
        if not local_layers:
            local_layers = list(range(len(self.block_len_per_layer)))
            self.local_registered_layer_indices = local_layers

        # Fallback names are used only by tests that bypass register_kv_caches.
        self.local_registered_layer_names = [
            f"model.layers.{layer_idx}.self_attn" for layer_idx in local_layers
        ]
        return self.local_registered_layer_names

    def _local_region_indices_for_layer_names(
        self, registered_layer_names: list[str]
    ) -> list[int]:
        local_names = self._ensure_local_registered_layer_names()
    ...
    def _kv_group_indices_for_layer_names(
        ...
        try:
            ...
            raise RuntimeError(
                f"KV cache layer {exc.args[0]!r} is not present in any "
                "kv_cache_group."
            ) from exc
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        caches_data = []
        registered_memory_base_addresses: set[int] = set()
        registered_base_addresses: list[int] = []
        ...
        for layer_name, cache_or_caches in xfer_buffers.items():
            ...
            for cache in cache_list:
                ...
                registered_base_addresses.append(base_addr)
                ...
                if base_addr not in registered_memory_base_addresses:
                    registered_memory_base_addresses.add(base_addr)
                    caches_data.append(
        ...
        assert (
            ...
            == len(registered_base_addresses)
            == len(registered_layer_names)
        )
        assert all(
            layer_index
            == _parse_registered_layer_index(
                registered_layer_names[i], start_layer, end_layer
            )
            for i, layer_index in enumerate(registered_layer_indices)
        )

        self._local_kv_cache_key = (pp_rank, self.tp_rank)
        self.local_registered_layer_indices = registered_layer_indices
        self.local_registered_layer_names = registered_layer_names
        self.kv_caches_base_addr[self.engine_id][self._local_kv_cache_key] = (
            registered_base_addresses
        )
        self.num_regions = len(registered_base_addresses)
        ...
        num_regions = len(registered_base_addresses)
        logger.info(
    ...
    def _register_local_xfer_handler_for_shard(
        self,
        engine_id: EngineId,
        remote_pp_rank: int,
        block_size: int,
        registered_layer_names: list[str] | None = None,
    ) -> tuple[int, list[tuple[int, int, int]], _ShardDescLayout]:
        handle_key = (engine_id, remote_pp_rank, block_size)
        if handle_key in self.src_xfer_handles_by_remote:
            layout = self._xfer_desc_layouts[(engine_id, remote_pp_rank, "local")]
            return (
                self.src_xfer_handles_by_remote[handle_key],
                self.src_blocks_data_by_remote[handle_key],
                layout,
            )
        ...
        if registered_layer_names is None:
            matching_meta = next(
                (
                    meta
                    for (pp_rank, _), meta in self._remote_agent_metadata[
                        engine_id
                    ].items()
                    if pp_rank == remote_pp_rank
                ),
                None,
            )
            if matching_meta is not None:
                registered_layer_names = matching_meta.registered_layer_names
            else:
                raise RuntimeError(
                    "Cannot register local xfer handler for engine "
                    f"{engine_id}, pp_rank {remote_pp_rank}: no "
                    "metadata available."
                )
        ...
        physical_region_group_ids = self._kv_group_indices_for_layer_names(
            registered_layer_names
        )
        local_base_addresses_all = self._get_local_base_addresses()
        if not local_base_addresses_all:
            local_base_addresses_all = [0] * len(self.block_len_per_layer)
        local_base_addresses = [
            local_base_addresses_all[local_idx] for local_idx in local_region_indices
        ]
        ...
        # Non-MLA/non-mamba split-K/V backends register K and V as separate
        # regions that share the same KV-group id, hence the x2 duplication.
        region_group_ids = (
            tuple(g for g in physical_region_group_ids for _ in range(2))
            if transfer_topo.is_kv_layout_blocks_first
            else physical_region_group_ids
        )
        mamba_region_group_ids = tuple(
            group_id for group_id in physical_region_group_ids for _ in range(4)
        )
        layout = _ShardDescLayout(
            num_regions=len(region_group_ids),
            num_blocks=self.num_blocks * block_size_ratio,
            physical_blocks_per_logical=self._physical_blocks_per_logical_kv_block,
            mamba_region_count=len(local_region_indices) * 4,
            region_group_ids=region_group_ids,
            mamba_region_group_ids=mamba_region_group_ids,
        )
        self.src_xfer_handles_by_remote[handle_key] = handle
        self.src_blocks_data_by_remote[handle_key] = blocks_data
        self._xfer_desc_layouts[(engine_id, remote_pp_rank, "local")] = layout
        return handle, blocks_data, layout
    ...
    def add_remote_agent(
        ...
        *,
        ...
        transfer_info = transfer_topo.register_remote_engine_shard(
        ...
        if self._has_mamba:
            ...
            if engine_id in self._physical_blocks_per_logical:
                assert (
                    self._physical_blocks_per_logical[engine_id]
                    == physical_blocks_per_logical
                ), (
                    "Mamba SSM geometry must be uniform across producer "
                    "PP shards for the same engine: "
                    f"engine_id={engine_id}, "
                    f"existing={self._physical_blocks_per_logical[engine_id]}, "
                    f"new={physical_blocks_per_logical}"
                )
            else:
                self._physical_blocks_per_logical[engine_id] = (
                    physical_blocks_per_logical
        ...
        logger.info(
            "Transfer plan: %s", transfer_topo.describe_shard(engine_id, remote_pp_rank)
        )
        ...
        _, src_blocks_data, local_desc_layout = (
            self._register_local_xfer_handler_for_shard(
                engine_id,
                remote_pp_rank,
        ...
        if (
            ...
            for handle_data in self._build_local_splits_from_plan(
                plan,
                src_blocks_data,
                local_desc_layout.num_fa_descs,
            ):
                descs = self.nixl_wrapper.get_xfer_descs(
                    handle_data, self.nixl_memory_type
                )
                handle = self.nixl_wrapper.prep_xfer_dlist("NIXL_INIT_AGENT", descs)
                self.src_xfer_handles_by_shard_tp_ratio[split_handle_key].append(
                    handle
                )
        ...
        remote_physical_region_group_ids = self._kv_group_indices_for_layer_names(
            nixl_agent_meta.registered_layer_names
        )
        remote_region_group_ids = (
            tuple(g for g in remote_physical_region_group_ids for _ in range(2))
            if transfer_topo.is_kv_layout_blocks_first
            else remote_physical_region_group_ids
        )
        remote_mamba_region_group_ids = tuple(
            group_id for group_id in remote_physical_region_group_ids for _ in range(4)
        )
        self._xfer_desc_layouts[(engine_id, remote_pp_rank, "remote")] = (
            _ShardDescLayout(
                num_regions=len(remote_region_group_ids),
                num_blocks=nixl_agent_meta.num_blocks,
                physical_blocks_per_logical=physical_blocks_per_logical,
                mamba_region_count=len(nixl_agent_meta.kv_caches_base_addr) * 4,
                region_group_ids=remote_region_group_ids,
                mamba_region_group_ids=remote_mamba_region_group_ids,
            )
        )
    ...
    def _validate_remote_agent_handshake(
        ...
        assert (
            0
            <= nixl_agent_meta.start_layer
            < nixl_agent_meta.end_layer
            <= total_layers
        )
        assert (
        ...
        for j, layer_name in enumerate(nixl_agent_meta.registered_layer_names):
            layer_index = _parse_registered_layer_index(
                layer_name,
                nixl_agent_meta.start_layer,
                nixl_agent_meta.end_layer,
            )
            assert nixl_agent_meta.registered_layer_indices[j] == layer_index
        # Will raise if any producer layer-name has no matching local region.
        local_region_indices = self._local_region_indices_for_layer_names(
            nixl_agent_meta.registered_layer_names
        )

        remote_info = self.transfer_topo.get_engine_shard_info(
        ...
        if not self.use_mla and not self._has_mamba:
            assert not (
                tp_ratio < 0
                and self.transfer_topo.is_kv_replicated_for_shard(
                    remote_engine_id, remote_pp_rank
                )
            )
        ...
        if (
            abs(tp_ratio) != 1
            and not self.use_mla
            and not self.transfer_topo.is_kv_replicated_for_shard(
                remote_engine_id, remote_pp_rank
            )
        ...
        remote_block_len = nixl_agent_meta.block_lens[0]
        remote_replicates_kv = self.transfer_topo.is_kv_replicated_for_shard(
            remote_engine_id, remote_pp_rank
        )
    ...
    def _read_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        first_remote_info = self.transfer_topo.get_engine_shard_info(engine_id, 0)
        meta.remote.block_ids = self._logical_to_remote_kernel_block_ids(
            meta.remote.block_ids,
            first_remote_info.remote_physical_blocks_per_logical,
        )
            ...
            for i, spec in enumerate(read_specs):
                ...
                if tp_ratio < 0 and not self.use_mla:
                    ...
                    local_xfer_side_handle = (
                        self.src_xfer_handles_by_shard_tp_ratio[split_key][i]
                    )
                else:
    ...
    def _get_block_descs_ids_for_shard(
        ...
        layout = self._xfer_desc_layouts[(engine_id, remote_pp_rank, side)]
        assert len(layout.region_group_ids) == layout.num_regions

        def descs_for_region_groups(
            region_group_ids: tuple[int, ...],
            num_blocks: int,
            offset: int = 0,
            include_mamba: bool | None = None,
        ) -> np.ndarray:
            desc_ids = []
            for region_id, group_id in enumerate(region_group_ids):
                assert group_id < len(block_ids)
                if include_mamba is not None:
                    is_mamba_group = _is_ssm_spec(self._group_spec_types[group_id])
                    if is_mamba_group != include_mamba:
                        continue
        ...
        if not self._has_mamba:
            return descs_for_region_groups(
                layout.region_group_ids, layout.num_blocks
            )

        physical_per_logical = layout.physical_blocks_per_logical
        assert physical_per_logical > 0
        assert layout.num_blocks % physical_per_logical == 0, (
            "Mamba descriptor layout num_blocks must be divisible by "
            "physical_blocks_per_logical"
        )
        assert layout.mamba_region_count > 0
        assert len(layout.mamba_region_group_ids) == layout.mamba_region_count
        logical_blocks = layout.num_blocks // physical_per_logical
        num_fa_descs = layout.num_fa_descs
        all_descs = [
            descs_for_region_groups(
                layout.region_group_ids,
                layout.num_blocks,
                include_mamba=False,
            ),
            descs_for_region_groups(
                layout.mamba_region_group_ids,
                logical_blocks,
                offset=num_fa_descs,
                include_mamba=True,
            ),
        ]
        all_descs = [desc_ids for desc_ids in all_descs if desc_ids.size > 0]
        if not all_descs:
            return np.empty(0, dtype=np.int64)
        descs_ids = np.concatenate(all_descs)
        num_descs = num_fa_descs + layout.mamba_region_count * logical_blocks
        assert descs_ids.size == 0 or int(descs_ids.max()) < num_descs, (
