...
class NixlPushConnectorWorker(NixlBaseConnectorWorker):
    ...
    def _xfer_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        if self.use_mla and tp_ratio < 0:
            # MLA latent KV is replicated across D's TP ranks. The tp-mapping
            # collapses to a single remote rank (valid when D *reads*, since
            # any one P replica suffices), but push must WRITE the latent into
            # every D rank this producer handshook or the un-written ranks
            # decode against stale KV. Fan the spec out across them; only the
            # dst handle differs per rank.
        ...
        # Track all of this request's WRITE handles in one locked update so the
        # engine thread's ``_pop_done_transfers`` never observes a partial set.
        # A request can fan out to several remote ranks (hetero-TP, and MLA
        # latent replicated to every D rank); appending per-write would let a
        # fast first write be reported finished-sending and freed before the
        # rest are tracked, then reported again as they complete (double free).
        if handles:
            with self._sending_transfers_lock:
