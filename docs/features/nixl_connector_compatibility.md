...
The default (pull) NixlConnector does **not** support `pipeline-parallel-size > 1` together with a hybrid KV cache (HMA) layout: region indices are not a stable identity across the prefill/decode layer split, so the connector raises at startup. Use the push connector (`NixlPushConnector`) for pipeline parallelism with hybrid or packed KV caches — it routes transfers by layer-name (member) identity, so a PP-sharded prefiller can write into a `PP=1` decoder. See [NIXL push-mode KV transfer](../design/nixl_kv_push_connector.md).
